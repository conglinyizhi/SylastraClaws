...
// PrintStatus renders sylastraclaws status (plain or fancy).
...
func printStatusPlain(r StatusReport) {
	fmt.Printf("%s sylastraclaws Status\n", r.Logo)
...
func printStatusFancy(r StatusReport) {
	...
	head.WriteString(titleBarStyle().Render(r.Logo + " sylastraclaws Status"))
	head.WriteString("\n\n")
