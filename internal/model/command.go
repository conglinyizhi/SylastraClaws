...
available list online and pick one), use the 'add' subcommand:
  ...
  sylastraclaws model add --help

Examples:
  sylastraclaws model                    # Show current default model
  sylastraclaws model gpt-5.2           # Set gpt-5.2 as default
  sylastraclaws model claude-sonnet-4.6 # Set claude-sonnet-4.6 as default
  sylastraclaws model local-model       # Set local VLLM server as default
  sylastraclaws model add -b URL -k KEY # Add a model from a custom endpoint
...
func showCurrentModel(cfg *config.Config) {
	...
	fmt.Println("\nTip: 'sylastraclaws model add -b URL -k KEY' adds a model from a custom")
	fmt.Println("     OpenAI-compatible endpoint (see 'sylastraclaws model add --help').")
}
