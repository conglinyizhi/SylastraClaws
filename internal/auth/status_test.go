...
import (
	...
	pkgauth "github.com/conglinyizhi/SylastraClaws/pkg/auth"
...
func setAuthStatusTestHome(t *testing.T) string {
	...
	tmpDir := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(tmpDir, ".sylastraclaws"))
	return tmpDir
}
...
func TestAuthStatusCmdShowsCanonicalGoogleAntigravityAfterLegacyRefresh(t *testing.T) {
	...
	authPath := filepath.Join(tmpDir, ".sylastraclaws", "auth.json")
