...
func authLoginGoogleAntigravity(noBrowser bool) error {
	...
	fmt.Println("Try it: sylastraclaws agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: sylastraclaws auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: sylastraclaws auth login --provider google-antigravity",
		)
	}
