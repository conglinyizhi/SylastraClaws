...
import (
	...
	"github.com/conglinyizhi/SylastraClaws/pkg/channels/weixin"
...
to authorize your account. On success, the bot token is saved to the sylastraclaws
...
func runWeixinOnboard(baseURL, proxy string, timeout time.Duration) error {
	...
	fmt.Println()
	fmt.Println("  sylastraclaws gateway")
	fmt.Println()
...
func printManualWeixinConfig(token, baseURL string) {
	fmt.Println()
	fmt.Println("Add the following to the channels section of your sylastraclaws config:")
	fmt.Println()
	fmt.Println(`  "weixin": {`)
