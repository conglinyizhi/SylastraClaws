...
import (
	...
	"github.com/conglinyizhi/SylastraClaws/pkg/credential"
)
...
func TestResolve_EncKey_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	sshKeyPath := filepath.Join(dir, "sylastraclaws_ed25519.key")
	...
	t.Setenv("SYLASTRACLAWS_SSH_KEY_PATH", sshKeyPath)
	...
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", passphrase)
...
func TestResolve_EncKey_WithSSHKey(t *testing.T) {
	...
	// Set SYLASTRACLAWS_SSH_KEY_PATH before Encrypt so the path passes allowedSSHKeyPath validation.
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", passphrase)
	t.Setenv("SYLASTRACLAWS_SSH_KEY_PATH", sshKeyPath)
...
func TestResolve_EncKey_NoPassphrase(t *testing.T) {
	...
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", "")

	r := credential.NewResolver(t.TempDir())
	_, err = r.Resolve(enc)
	if err == nil {
		t.Fatal("expected error when SYLASTRACLAWS_KEY_PASSPHRASE is unset, got nil")
	}
}

func TestResolve_EncKey_BadCiphertext(t *testing.T) {
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", "some-passphrase")
	t.Setenv("SYLASTRACLAWS_SSH_KEY_PATH", "")
...
func TestResolve_EncKey_WrongPassphrase(t *testing.T) {
	...
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", "wrong-passphrase")
...
func TestDeriveKey_SSHKeyNotFound(t *testing.T) {
	...
	t.Setenv("SYLASTRACLAWS_KEY_PASSPHRASE", "passphrase")
	t.Setenv("SYLASTRACLAWS_SSH_KEY_PATH", filepath.Join(dir, "nonexistent_key"))
...
// that are not under SYLASTRACLAWS_SSH_KEY_PATH, SYLASTRACLAWS_HOME, or ~/.ssh/.
