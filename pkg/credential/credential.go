...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via SYLASTRACLAWS_KEY_PASSPHRASE
...
//  2. SYLASTRACLAWS_SSH_KEY_PATH env var
//  3. ~/.ssh/sylastraclaws_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "SYLASTRACLAWS_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading SYLASTRACLAWS_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "SYLASTRACLAWS_SSH_KEY_PATH"

// sylastraClawsHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const sylastraClawsHome = "SYLASTRACLAWS_HOME"

const (
	FileScheme = "file://"
	EncScheme  = "enc://"

	hkdfInfo = "sylastraclaws-credential-v1"
	saltLen  = 16
	nonceLen = 12
	keyLen   = 32
)
...
//
// passphrase is required (SYLASTRACLAWS_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// SYLASTRACLAWS_SSH_KEY_PATH env var or ~/.ssh/sylastraclaws_ed25519.key.
...
//   - exact match with SYLASTRACLAWS_SSH_KEY_PATH env var
//   - within the SYLASTRACLAWS_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with SYLASTRACLAWS_SSH_KEY_PATH.
	...
	// Within SYLASTRACLAWS_HOME.
	if sHome := os.Getenv(sylastraClawsHome); sHome != "" {
		if isWithinDir(clean, sHome) {
			return true
		}
	}
...
// Final key: HKDF-SHA256(ikm, salt, info="sylastraclaws-credential-v1", 32 bytes)
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set SYLASTRACLAWS_SSH_KEY_PATH or place key at ~/.ssh/sylastraclaws_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (SYLASTRACLAWS_SSH_KEY_PATH, SYLASTRACLAWS_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. SYLASTRACLAWS_SSH_KEY_PATH env var
//  3. ~/.ssh/sylastraclaws_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the SylastraClaws-specific SSH key path if it exists.
