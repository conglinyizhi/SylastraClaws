...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("SYLASTRACLAWS_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"SYLASTRACLAWS_HOOK_HELPER=1",
		"SYLASTRACLAWS_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "SYLASTRACLAWS_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("SYLASTRACLAWS_HOOK_MODE")
	eventLog := os.Getenv("SYLASTRACLAWS_HOOK_EVENT_LOG")
