...
File: ~/.config/sylastraclaws/.security.yml
...
File: ~/.config/sylastraclaws/config.json
...
Edit ~/.config/sylastraclaws/.security.yml and replace placeholders with your actual keys.

## Step 4: Simplify config.json (Recommended)
Remove sensitive fields from ~/.config/sylastraclaws/config.json:
...
export SYLASTRACLAWS_CHANNELS_DISCORD_TOKEN="discord-token-from-env"

# Web Tools
export SYLASTRACLAWS_TOOLS_WEB_BRAVE_API_KEY="brave-key-from-env"
export SYLASTRACLAWS_TOOLS_WEB_BAIDU_API_KEY="baidu-key-from-env"

# Skills
export SYLASTRACLAWS_TOOLS_SKILLS_GITHUB_TOKEN="github-token-from-env"
```
