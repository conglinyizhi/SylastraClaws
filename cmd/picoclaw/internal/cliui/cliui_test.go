...
func TestStyleUsageTokensContainsTokens(t *testing.T) {
	cases := []struct {
		...
		{
			"sylastraclaws agent <message>",
			[]string{"sylastraclaws agent", "<message>"},
		},
		{
			"sylastraclaws [command] [flags]",
			[]string{"sylastraclaws", "[command]", "[flags]"},
		},
		{
			"sylastraclaws",
			[]string{"sylastraclaws"},
		},
		{
			"cmd <arg1> [--flag]",
