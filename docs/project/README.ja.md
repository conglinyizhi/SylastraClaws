<div align="center">
  ...
  <h1>SylastraClaws: Go で書かれた超効率 AI アシスタント</h1>

  <h3>$10 ハードウェア · 10MB RAM · ms 起動 · Let's Go, SylastraClaws!</h3>
  <p>
...
> **SylastraClaws** は [Sipeed](https://sipeed.com) が立ち上げた独立したオープンソースプロジェクトです。完全に **Go 言語**で一から書かれており、OpenClaw、NanoBot、その他のプロジェクトのフォークではありません。

**SylastraClaws** は [NanoBot](https://github.com/HKUDS/nanobot) にインスパイアされた超軽量パーソナル AI アシスタントです。**Go** でゼロからリビルドされ、「セルフブートストラッピング」プロセスで構築されました — AI Agent 自身がアーキテクチャの移行とコード最適化を推進しました。
...
> **セキュリティに関する注意**
>
> * **暗号通貨なし:** SylastraClaws には公式トークン/コインは**一切ありません**。`pump.fun` やその他の取引プラットフォームでの主張はすべて**詐欺**です。
> * **公式ドメイン:** **唯一**の公式サイトは **[sylastraclaws.io](https://sylastraclaws.io)**、企業サイトは **[sipeed.com](https://sipeed.com)** です。
> * **注意:** 多くの `.ai/.org/.com/.net/...` ドメインは第三者によって登録されています。信頼しないでください。
> * **注記:** SylastraClaws は初期開発段階にあり、未解決のネットワークセキュリティ問題がある可能性があります。v1.0 リリース前に本番環境へのデプロイは避けてください。
> * **注記:** SylastraClaws は最近多くの PR をマージしており、最新バージョンではメモリフットプリントが大きくなる場合があります（10〜20MB）。機能セットが安定次第、リソース最適化を優先する予定です。

## 📢 ニュース

2026-03-31 📱 **Android サポート！** SylastraClawsがAndroidで動作！APKは[sylastraclaws.io](https://sylastraclaws.io/download)からダウンロード

2026-03-25 🚀 **v0.2.4 リリース！** Agent アーキテクチャ全面刷新（SubTurn、Hooks、Steering、EventBus）、WeChat/WeCom 統合、セキュリティ強化（.security.yml、機密データフィルタリング）、新プロバイダー（AWS Bedrock、Azure、Xiaomi MiMo）、35 件のバグ修正。SylastraClaws **26K ⭐** 達成！

2026-03-17 🚀 **v0.2.3 リリース！** システムトレイ UI（Windows & Linux）、サブエージェントステータス追跡（`spawn_status`）、実験的 Gateway ホットリロード、cron セキュリティゲート、セキュリティ修正 2 件。SylastraClaws **25K ⭐** 達成！
...
2026-02-26 🎉 SylastraClaws がわずか 17 日で **20K スター** 達成！Channel 自動オーケストレーションとケイパビリティインターフェースが実装されました。

2026-02-16 🎉 SylastraClaws が 1 週間で 12K スター達成！コミュニティメンテナーの役割と[ロードマップ](../../ROADMAP.md)が正式に公開されました。

2026-02-13 🎉 SylastraClaws が 4 日間で 5000 スター達成！プロジェクトロードマップと開発者グループの準備が進行中。

2026-02-09 🎉 **SylastraClaws リリース！** $10 ハードウェアで 10MB 未満の RAM で動く AI Agent を 1 日で構築。Let's Go, SylastraClaws!
...
SylastraClaws はほぼすべての Linux デバイスにデプロイできます！
...
### sylastraclaws.io からダウンロード（推奨）

**[sylastraclaws.io](https://sylastraclaws.io)** にアクセス — 公式サイトがプラットフォームを自動検出し、ワンクリックでダウンロードできます。アーキテクチャを手動で選ぶ必要はありません。

### プリコンパイル済みバイナリをダウンロード

または、[GitHub Releases](https://github.com/conglinyizhi/SylastraClaws/releases) ページからプラットフォームに合ったバイナリをダウンロードしてください。
...
[sylastraclaws.io](https://sylastraclaws.io) からダウンロード後、`sylastraclaws-launcher`（Windows では `sylastraclaws-launcher.exe`）をダブルクリックしてください。ブラウザが自動的に `http://localhost:18800` を開きます。
...
WebUI の詳細なドキュメントは [docs.sylastraclaws.io](https://docs.sylastraclaws.io) を参照してください。
...
> **Docker / VM ユーザー:** Gateway はデフォルトで `127.0.0.1` でリッスンします。ホストからアクセスできるようにするには `SYLASTRACLAWS_GATEWAY_HOST=0.0.0.0` を設定するか、`-public` フラグを使用してください。
...
`sylastraclaws-launcher` はインターネットからダウンロードされ、Mac App Store を通じて公証されていないため、macOS が初回起動時にブロックする場合があります。

**ステップ 1：** `sylastraclaws-launcher` をダブルクリックすると、セキュリティ警告が表示されます：

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper 警告" width="400">
</p>

> *"sylastraclaws-launcher" は開けません — "sylastraclaws-launcher" がMacに害を与えたりプライバシーを侵害するマルウェアを含まないことをAppleは確認できません。*
...
10 年前のスマホに第二の人生を！SylastraClaws でスマート AI アシスタントに変身させましょう。
...
[sylastraclaws.io](https://sylastraclaws.io/download/) から APK をダウンロードして直接インストール。Termux 不要！
...
pkg install proot
termux-chroot ./sylastraclaws onboard   # chroot で標準的な Linux ファイルシステムレイアウトを提供
```
...
`sylastraclaws` コアバイナリのみが利用可能な最小環境（Launcher UI なし）では、コマンドラインと JSON 設定ファイルですべてを設定できます。
...
`~/.config/sylastraclaws/config.json` とワークスペースディレクトリが作成されます。

**2. 設定** (`~/.config/sylastraclaws/config.json`)
...
SylastraClaws は `model_list` 設定を通じて 30 以上の LLM Provider をサポートしています。`protocol/model` 形式を使用してください：
...
17 以上のメッセージングプラットフォームで SylastraClaws と会話できます：
...
> ログの詳細度は `gateway.log_level` で制御します（デフォルト：`warn`）。サポートされる値：`debug`、`info`、`warn`、`error`、`fatal`。`SYLASTRACLAWS_LOG_LEVEL` 環境変数でも設定可能です。詳細は[設定ガイド](../guides/configuration.ja.md#gateway-ログレベル)を参照してください。
...
SylastraClaws は最新情報を提供するために Web を検索できます。`tools.web` で設定してください：
...
SylastraClaws にはファイル操作、コード実行、スケジューリングなどの組み込みツールが含まれています。詳細は [ツール設定](../reference/tools_configuration.ja.md) を参照してください。
...
SylastraClaws は [MCP](https://modelcontextprotocol.io/) をネイティブサポートしています — 任意の MCP サーバーに接続して、外部ツールやデータソースで Agent の機能を拡張できます。
...
CLI または統合チャットアプリからメッセージを 1 つ送るだけで、SylastraClaws をエージェントソーシャルネットワークに接続できます。
...
| コマンド                  | 説明                           |
| ------------------------- | ------------------------------ |
| `sylastraclaws onboard`        | 設定＆ワークスペースの初期化     |
| `sylastraclaws auth weixin` | WeChat アカウントを QR で接続 |
| `sylastraclaws agent -m "..."` | Agent とチャット                |
| `sylastraclaws agent`          | インタラクティブチャットモード   |
| `sylastraclaws gateway`        | Gateway を起動                  |
| `sylastraclaws status`         | ステータスを表示                |
| `sylastraclaws version`        | バージョン情報を表示            |
| `sylastraclaws model`          | デフォルトモデルの表示・切替    |
| `sylastraclaws cron list`      | スケジュールジョブ一覧          |
| `sylastraclaws cron add ...`   | スケジュールジョブを追加         |
| `sylastraclaws cron disable`   | スケジュールジョブを無効化       |
| `sylastraclaws cron remove`    | スケジュールジョブを削除         |
| `sylastraclaws skills list`    | インストール済み Skill 一覧      |
| `sylastraclaws skills install` | Skill をインストール             |
| `sylastraclaws migrate`        | 旧バージョンからデータを移行     |
| `sylastraclaws auth login`     | Provider への認証               |

### ⏰ スケジュールタスク / リマインダー

SylastraClaws は `cron` ツールによるスケジュールリマインダーと定期タスクをサポートしています：
...
[コミュニティロードマップ](https://github.com/conglinyizhi/SylastraClaws/issues/988)と[CONTRIBUTING.md](../../CONTRIBUTING.md)をご覧ください。
