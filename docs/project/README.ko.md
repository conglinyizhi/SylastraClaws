...
<h1>SylastraClaws: Go로 작성된 초고효율 AI 어시스턴트</h1>

<h3>$10 하드웨어 · 10MB RAM · ms 부팅 · Let's Go, SylastraClaws!</h3>
  <p>
...
> **SylastraClaws**는 [Sipeed](https://sipeed.com)가 시작한 독립적인 오픈소스 프로젝트입니다. 처음부터 끝까지 **Go**로 새로 작성되었으며, OpenClaw, NanoBot, 혹은 다른 어떤 프로젝트의 포크도 아닙니다.

**SylastraClaws**는 [NanoBot](https://github.com/HKUDS/nanobot)에서 영감을 받은 초경량 개인용 AI 어시스턴트입니다. **Go**로 처음부터 다시 구현되었고, "셀프 부트스트래핑" 방식으로 만들어졌습니다. 즉, AI 에이전트 자체가 아키텍처 전환과 코드 최적화를 주도했습니다.
...
> **보안 안내**
>
> * **암호화폐 없음:** SylastraClaws는 공식 토큰이나 암호화폐를 **발행한 적이 없습니다**. `pump.fun` 또는 기타 거래 플랫폼에서의 모든 주장은 **사기**입니다.
> * **공식 도메인:** **유일한** 공식 웹사이트는 **[sylastraclaws.io](https://sylastraclaws.io)** 이며, 회사 웹사이트는 **[sipeed.com](https://sipeed.com)** 입니다.
> * **주의:** 많은 `.ai/.org/.com/.net/...` 도메인이 제3자에 의해 등록되어 있습니다. 신뢰하지 마세요.
> * **참고:** SylastraClaws는 빠르게 초기 개발이 진행 중입니다. 아직 해결되지 않은 보안 문제가 있을 수 있습니다. v1.0 이전에는 프로덕션 배포를 권장하지 않습니다.
> * **참고:** SylastraClaws는 최근 많은 PR을 병합했습니다. 최근 빌드는 10~20MB RAM을 사용할 수 있습니다. 기능이 안정화된 뒤 리소스 최적화를 진행할 예정입니다.

## 📢 뉴스

2026-03-31 📱 **Android 지원!** SylastraClaws가 이제 Android에서 실행됩니다! APK는 [sylastraclaws.io](https://sylastraclaws.io/download)에서 다운로드하세요.

2026-03-25 🚀 **v0.2.4 출시!** 에이전트 아키텍처 전면 개편(SubTurn, Hooks, Steering, EventBus), WeChat/WeCom 통합, 보안 강화(`.security.yml`, 민감 정보 필터링), 새 프로바이더(AWS Bedrock, Azure, Xiaomi MiMo), 그리고 35건의 버그 수정이 포함되었습니다. SylastraClaws는 **26K 스타**를 달성했습니다!

2026-03-17 🚀 **v0.2.3 출시!** 시스템 트레이 UI(Windows 및 Linux), 서브에이전트 상태 조회(`spawn_status`), 실험적 게이트웨이 핫 리로드, Cron 보안 게이트, 그리고 2건의 보안 수정이 추가되었습니다. SylastraClaws는 **25K 스타**를 달성했습니다!
...
2026-02-26 🎉 SylastraClaws가 단 17일 만에 **20K 스타**를 달성했습니다! 채널 자동 오케스트레이션과 기능 인터페이스가 적용되었습니다.

2026-02-16 🎉 SylastraClaws가 1주일 만에 **12K 스타**를 돌파했습니다! 커뮤니티 메인터너 역할과 [로드맵](../../ROADMAP.md)이 공식적으로 공개되었습니다.

2026-02-13 🎉 SylastraClaws가 4일 만에 **5000 스타**를 돌파했습니다! 프로젝트 로드맵과 개발자 그룹이 준비 중입니다.

2026-02-09 🎉 **SylastraClaws 출시!** $10 하드웨어와 10MB 미만 RAM에서 동작하는 AI 에이전트를 단 1일 만에 만들었습니다. Let's Go, SylastraClaws!
...
SylastraClaws는 사실상 거의 모든 Linux 장치에 배포할 수 있습니다!
...
### sylastraclaws.io에서 다운로드(권장)

**[sylastraclaws.io](https://sylastraclaws.io)** 를 방문하세요. 공식 웹사이트가 플랫폼을 자동 감지하고 원클릭 다운로드를 제공합니다. 아키텍처를 직접 고를 필요가 없습니다.

### 사전 컴파일된 바이너리 다운로드

또는 [GitHub Releases](https://github.com/conglinyizhi/SylastraClaws/releases) 페이지에서 플랫폼에 맞는 바이너리를 다운로드할 수 있습니다.
...
[sylastraclaws.io](https://sylastraclaws.io)에서 다운로드한 뒤 `sylastraclaws-launcher`를 더블클릭하세요(Windows에서는 `sylastraclaws-launcher.exe`). 브라우저가 자동으로 `http://localhost:18800`을 엽니다.
...
자세한 WebUI 문서는 [docs.sylastraclaws.io](https://docs.sylastraclaws.io)를 참고하세요.
...
> **Docker / VM 사용자:** 게이트웨이는 기본적으로 `127.0.0.1`에서 수신합니다. 호스트에서 접근 가능하게 하려면 `SYLASTRACLAWS_GATEWAY_HOST=0.0.0.0`을 설정하거나 `-public` 플래그를 사용하세요.
...
macOS에서는 인터넷에서 다운로드한 앱이고 Mac App Store 공증을 거치지 않았기 때문에, 첫 실행 시 `sylastraclaws-launcher`가 차단될 수 있습니다.

**1단계:** `sylastraclaws-launcher`를 더블클릭합니다. 그러면 보안 경고가 표시됩니다.

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="macOS Gatekeeper warning" width="400">
</p>

> *"sylastraclaws-launcher"을(를) 열 수 없습니다. Apple에서 이 앱이 악성 소프트웨어가 없으며 Mac이나 개인 정보를 해치지 않는다고 확인할 수 없습니다.*
...
이 과정을 한 번만 거치면 이후에는 `sylastraclaws-launcher`가 정상적으로 열립니다.

</details>

### 📱 Android

오래된 스마트폰에 새 생명을 불어넣어 보세요! SylastraClaws를 설치하면 스마트 AI 어시스턴트로 바꿀 수 있습니다.
...
[sylastraclaws.io](https://sylastraclaws.io/download/)에서 APK를 다운로드해 바로 설치하세요. Termux가 필요 없습니다!
...
pkg install proot
termux-chroot ./sylastraclaws onboard   # chroot가 표준 Linux 파일시스템 레이아웃을 제공합니다
```
...
런처 UI 없이 `sylastraclaws` 코어 바이너리만 있는 최소 환경에서는 명령줄과 JSON 설정 파일만으로도 모든 설정을 마칠 수 있습니다.
...
그러면 `~/.config/sylastraclaws/config.json`과 워크스페이스 디렉터리가 생성됩니다.

**2. 설정** (`~/.config/sylastraclaws/config.json`)
...
```bash
# 단발성 질문
sylastraclaws agent -m "2+2는 얼마야?"
...
SylastraClaws는 `model_list` 설정을 통해 30개 이상의 LLM 프로바이더를 지원합니다. 형식은 `protocol/model`입니다.
...
18개 이상의 메시징 플랫폼을 통해 SylastraClaws와 대화할 수 있습니다.
...
> 로그 상세도는 `gateway.log_level`(기본값: `warn`)로 제어됩니다. 지원 값은 `debug`, `info`, `warn`, `error`, `fatal`입니다. `SYLASTRACLAWS_LOG_LEVEL` 환경 변수로도 설정할 수 있습니다. 자세한 내용은 [설정 문서](../guides/configuration.md#gateway-log-level)를 참고하세요.
...
SylastraClaws는 최신 정보를 제공하기 위해 웹 검색을 수행할 수 있습니다. `tools.web`에서 설정하세요.
...
SylastraClaws에는 파일 작업, 코드 실행, 스케줄링 등을 위한 내장 도구가 포함되어 있습니다. 자세한 내용은 [도구 설정](../reference/tools_configuration.md)을 참고하세요.
...
SylastraClaws는 [MCP](https://modelcontextprotocol.io/)를 기본 지원합니다. 어떤 MCP 서버든 연결하여 외부 도구와 데이터 소스로 에이전트 기능을 확장할 수 있습니다.
...
CLI 또는 통합된 채팅 앱에서 메시지를 한 번만 보내면 SylastraClaws를 에이전트 소셜 네트워크에 연결할 수 있습니다.
...
| 명령어                    | 설명                           |
| ------------------------- | ------------------------------ |
| `sylastraclaws onboard`        | 설정 및 워크스페이스 초기화    |
| `sylastraclaws auth weixin`    | QR로 WeChat 계정 연결          |
| `sylastraclaws agent -m "..."` | 에이전트와 채팅               |
| `sylastraclaws agent`          | 대화형 채팅 모드               |
| `sylastraclaws gateway`        | 게이트웨이 시작                |
| `sylastraclaws status`         | 상태 표시                      |
| `sylastraclaws version`        | 버전 정보 표시                 |
| `sylastraclaws model`          | 기본 모델 조회 또는 변경       |
| `sylastraclaws cron list`      | 모든 예약 작업 목록 표시       |
| `sylastraclaws cron add ...`   | 예약 작업 추가                 |
| `sylastraclaws cron disable`   | 예약 작업 비활성화             |
| `sylastraclaws cron remove`    | 예약 작업 삭제                 |
| `sylastraclaws skills list`    | 설치된 스킬 목록 표시          |
| `sylastraclaws skills install` | 스킬 설치                      |
| `sylastraclaws migrate`        | 이전 버전 데이터 마이그레이션  |
| `sylastraclaws auth login`     | 프로바이더 인증                |

### ⏰ 예약 작업 / 리마인더

SylastraClaws는 `cron` 도구를 통해 예약 리마인더와 반복 작업을 지원합니다.
...
가이드라인은 [커뮤니티 로드맵](https://github.com/conglinyizhi/SylastraClaws/issues/988)과 [CONTRIBUTING.md](../../CONTRIBUTING.md)를 참고하세요.
