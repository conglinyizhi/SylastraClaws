...
<h1>SylastraClaws: Assistente IA Ultra-Efficiente in Go</h1>

<h3>Hardware da $10 · 10MB di RAM · Avvio in ms · Let's Go, SylastraClaws!</h3>
  <p>
...
> **SylastraClaws** è un progetto open-source indipendente avviato da [Sipeed](https://sipeed.com), scritto interamente in **Go** da zero — non è un fork di OpenClaw, NanoBot o di qualsiasi altro progetto.

**SylastraClaws** è un assistente IA personale ultra-leggero ispirato a [NanoBot](https://github.com/HKUDS/nanobot). È stato riscritto da zero in **Go** attraverso un processo di "auto-bootstrapping" — l'Agent IA stesso ha guidato la migrazione architetturale e l'ottimizzazione del codice.
...
> **Avviso di Sicurezza**
>
> * **NESSUNA CRYPTO:** SylastraClaws **non** ha emesso token o criptovalute ufficiali. Qualsiasi annuncio su `pump.fun` o altre piattaforme di trading è una **truffa**.
> * **DOMINIO UFFICIALE:** L'**UNICO** sito ufficiale è **[sylastraclaws.io](https://sylastraclaws.io)**, e il sito aziendale è **[sipeed.com](https://sipeed.com)**
> * **ATTENZIONE:** Molti domini `.ai/.org/.com/.net/...` sono stati registrati da terze parti. Non fidarti di essi.
> * **NOTA:** SylastraClaws è in fase di sviluppo iniziale rapido. Potrebbero esserci problemi di sicurezza non risolti. Non distribuire in produzione prima della v1.0.
> * **NOTA:** SylastraClaws ha recentemente unito molte PR. Le build recenti potrebbero usare 10-20MB di RAM. L'ottimizzazione delle risorse è pianificata dopo la stabilizzazione delle funzionalità.

## 📢 Novità

2026-03-31 📱 **Supporto Android!** SylastraClaws ora funziona su Android! Scarica l'APK su [sylastraclaws.io](https://sylastraclaws.io/download)

2026-03-25 🚀 **v0.2.4 rilasciata!** Revisione dell'architettura Agent (SubTurn, Hooks, Steering, EventBus), integrazione WeChat/WeCom, rafforzamento della sicurezza (.security.yml, filtraggio dati sensibili), nuovi provider (AWS Bedrock, Azure, Xiaomi MiMo) e 35 correzioni di bug. SylastraClaws raggiunge **26K Stars**!

2026-03-17 🚀 **v0.2.3 rilasciata!** Interfaccia system tray (Windows & Linux), query sullo stato dei sub-agent (`spawn_status`), hot-reload sperimentale del Gateway, gate di sicurezza per Cron e 2 correzioni di sicurezza. SylastraClaws raggiunge **25K Stars**!
...
2026-02-26 🎉 SylastraClaws raggiunge **20K stelle** in soli 17 giorni! Orchestrazione automatica dei canali e interfacce di capacità sono attive.

2026-02-16 🎉 SylastraClaws supera 12K stelle in una settimana! Ruoli di maintainer della community e [Roadmap](../../ROADMAP.md) pubblicati ufficialmente.

2026-02-13 🎉 SylastraClaws supera 5000 stelle in 4 giorni! Roadmap del progetto e gruppi sviluppatori in fase di avvio.

2026-02-09 🎉 **SylastraClaws lanciato!** Costruito in 1 giorno per portare gli AI Agent su hardware da $10 con <10MB di RAM. Let's Go, SylastraClaws!
...
<p align="center">
<img src="../../assets/hardware-banner.jpg" alt="SylastraClaws Hardware Compatibility" width="100%">
</p>
...
SylastraClaws può essere distribuito su quasi qualsiasi dispositivo Linux!
...
### Scarica da sylastraclaws.io (Consigliato)

Visita **[sylastraclaws.io](https://sylastraclaws.io)** — il sito ufficiale rileva automaticamente la tua piattaforma e fornisce il download con un clic. Non è necessario scegliere manualmente l'architettura.

### Scarica il binario precompilato

In alternativa, scarica il binario per la tua piattaforma dalla pagina delle [GitHub Releases](https://github.com/conglinyizhi/SylastraClaws/releases).
...
Dopo aver scaricato da [sylastraclaws.io](https://sylastraclaws.io), fai doppio clic su `sylastraclaws-launcher` (o `sylastraclaws-launcher.exe` su Windows). Il browser si aprirà automaticamente su `http://localhost:18800`.
...
Per la documentazione dettagliata del WebUI, vedi [docs.sylastraclaws.io](https://docs.sylastraclaws.io).
...
> **Utenti Docker / VM:** Il Gateway ascolta su `127.0.0.1` per impostazione predefinita. Imposta `SYLASTRACLAWS_GATEWAY_HOST=0.0.0.0` o usa il flag `-public` per renderlo accessibile dall'host.
...
macOS potrebbe bloccare `sylastraclaws-launcher` al primo avvio perché è stato scaricato da internet e non è notarizzato tramite il Mac App Store.

**Passo 1:** Fai doppio clic su `sylastraclaws-launcher`. Verrà visualizzato un avviso di sicurezza:

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="Avviso macOS Gatekeeper" width="400">
</p>

> *"sylastraclaws-launcher" Non Aperto — Apple non è riuscita a verificare che "sylastraclaws-launcher" sia privo di malware che potrebbe danneggiare il Mac o compromettere la privacy.*
...
Dopo questo passaggio una tantum, `sylastraclaws-launcher` si aprirà normalmente ai lanci successivi.

</details>

### 📱 Android

Dai una seconda vita al tuo telefono di dieci anni fa! Trasformalo in un assistente IA intelligente con SylastraClaws.
...
Scarica l'APK da [sylastraclaws.io](https://sylastraclaws.io/download/) e installa direttamente. Senza Termux!
...
pkg install proot
termux-chroot ./sylastraclaws onboard   # chroot fornisce un layout standard del filesystem Linux
```

Poi segui la sezione Terminal Launcher qui sotto per completare la configurazione.

<img src="../../assets/termux.jpg" alt="SylastraClaws on Termux" width="512">

Per ambienti minimali dove è disponibile solo il binario core `sylastraclaws` (senza Launcher UI), puoi configurare tutto tramite riga di comando e un file di configurazione JSON.
...
Questo crea `~/.config/sylastraclaws/config.json` e la directory workspace.

**2. Configura** (`~/.config/sylastraclaws/config.json`)
...
```bash
# Domanda singola
sylastraclaws agent -m "Quanto fa 2+2?"
...
SylastraClaws supporta 30+ provider LLM tramite la configurazione `model_list`. Usa il formato `protocollo/modello`:
...
Parla con il tuo SylastraClaws attraverso 17+ piattaforme di messaggistica:
...
> La verbosità dei log è controllata da `gateway.log_level` (default: `warn`). Valori supportati: `debug`, `info`, `warn`, `error`, `fatal`. Può essere impostato anche tramite `SYLASTRACLAWS_LOG_LEVEL`. Vedi [Configurazione](../guides/configuration.md#gateway-log-level) per i dettagli.
...
SylastraClaws può cercare sul web per fornire informazioni aggiornate. Configura in `tools.web`:
...
SylastraClaws include strumenti integrati per operazioni su file, esecuzione di codice, pianificazione e altro. Vedi [Configurazione degli Strumenti](../reference/tools_configuration.md) per i dettagli.
...
sylastraclaws skills install <skill-name>
```
...
SylastraClaws supporta nativamente [MCP](https://modelcontextprotocol.io/) — connetti qualsiasi server MCP per estendere le capacità del tuo Agent con strumenti e sorgenti di dati esterni.
...
```bash
sylastraclaws mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /tmp
sylastraclaws mcp list
sylastraclaws mcp test filesystem
```

`sylastraclaws mcp` agisce come configuration manager: aggiorna `config.json` sotto `tools.mcp.servers`, ma non mantiene in esecuzione il processo del server.

Usa `sylastraclaws mcp edit` quando ti servono campi avanzati che non sono coperti da `sylastraclaws mcp add`.
Per esempio, `sylastraclaws mcp add` supporta `--deferred` e `--env-file`, mentre `sylastraclaws mcp edit` resta utile per modifiche JSON dirette e opzioni MCP meno comuni.
...
Connetti SylastraClaws al Social Network degli Agent semplicemente inviando un singolo messaggio tramite CLI o qualsiasi app di chat integrata.
...
| Comando                   | Descrizione                        |
| ------------------------- | ---------------------------------- |
| `sylastraclaws onboard`        | Inizializza config & workspace     |
| `sylastraclaws auth weixin` | Connetti account WeChat tramite QR |
| `sylastraclaws agent -m "..."` | Chatta con l'agent                 |
| `sylastraclaws agent`          | Modalità chat interattiva          |
| `sylastraclaws gateway`        | Avvia il gateway                   |
| `sylastraclaws status`         | Mostra lo stato                    |
| `sylastraclaws version`        | Mostra le info sulla versione      |
| `sylastraclaws model`          | Visualizza o cambia il modello predefinito |
| `sylastraclaws mcp list`       | Elenca i server MCP configurati    |
| `sylastraclaws mcp add ...`    | Aggiunge o aggiorna un server MCP  |
| `sylastraclaws mcp test`       | Verifica la raggiungibilità di un server MCP |
| `sylastraclaws mcp edit`       | Apre la config per modifiche MCP avanzate |
| `sylastraclaws mcp remove`     | Rimuove un server MCP dalla config |
| `sylastraclaws cron list`      | Elenca tutti i job pianificati     |
| `sylastraclaws cron add ...`   | Aggiunge un job pianificato        |
| `sylastraclaws cron disable`   | Disabilita un job pianificato      |
| `sylastraclaws cron remove`    | Rimuove un job pianificato         |
| `sylastraclaws skills list`    | Elenca le skill installate         |
| `sylastraclaws skills install` | Installa una skill                 |
| `sylastraclaws migrate`        | Migra i dati dalle versioni precedenti |
| `sylastraclaws auth login`     | Autenticazione con i provider          |

### ⏰ Task Pianificati / Promemoria

SylastraClaws supporta promemoria pianificati e task ricorrenti tramite lo strumento `cron`:
...
Consulta la nostra [Roadmap della Community](https://github.com/conglinyizhi/SylastraClaws/issues/988) e [CONTRIBUTING.md](../../CONTRIBUTING.md) per le linee guida.
