...
<h1>SylastraClaws: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>

<h3>Phần cứng $10 · RAM 10MB · Khởi động ms · Let's Go, SylastraClaws!</h3>
  <p>
...
> **SylastraClaws** là một dự án mã nguồn mở độc lập do [Sipeed](https://sipeed.com) khởi xướng, được viết hoàn toàn bằng **Go** từ đầu — không phải fork của OpenClaw, NanoBot hay bất kỳ dự án nào khác.

**SylastraClaws** là trợ lý AI cá nhân siêu nhẹ lấy cảm hứng từ [NanoBot](https://github.com/HKUDS/nanobot). Nó được xây dựng lại từ đầu bằng **Go** thông qua quá trình "tự khởi động" — chính AI Agent đã dẫn dắt quá trình di chuyển kiến trúc và tối ưu hóa mã nguồn.
...
> **Thông báo Bảo mật**
>
> * **KHÔNG CÓ CRYPTO:** SylastraClaws **chưa** phát hành bất kỳ token hay tiền điện tử chính thức nào. Mọi thông tin trên `pump.fun` hoặc các nền tảng giao dịch khác đều là **lừa đảo**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[sylastraclaws.io](https://sylastraclaws.io)**, và website công ty là **[sipeed.com](https://sipeed.com)**
> * **CẢNH BÁO:** Nhiều domain `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký. Đừng tin tưởng chúng.
> * **LƯU Ý:** SylastraClaws đang trong giai đoạn phát triển nhanh. Có thể còn các vấn đề bảo mật chưa được giải quyết. Không triển khai lên môi trường production trước v1.0.
> * **LƯU Ý:** SylastraClaws gần đây đã merge nhiều PR. Các bản build gần đây có thể dùng 10-20MB RAM. Tối ưu hóa tài nguyên được lên kế hoạch sau khi tính năng ổn định.

## 📢 Tin tức

2026-03-31 📱 **Hỗ trợ Android!** SylastraClaws giờ chạy trên Android! Tải APK tại [sylastraclaws.io](https://sylastraclaws.io/download)

2026-03-25 🚀 **v0.2.4 đã phát hành!** Tái cấu trúc kiến trúc Agent (SubTurn, Hooks, Steering, EventBus), tích hợp WeChat/WeCom, tăng cường bảo mật (.security.yml, lọc dữ liệu nhạy cảm), provider mới (AWS Bedrock, Azure, Xiaomi MiMo) và 35 bản vá lỗi. SylastraClaws đã đạt **26K Stars**!

2026-03-17 🚀 **v0.2.3 đã phát hành!** Giao diện system tray (Windows & Linux), truy vấn trạng thái sub-agent (`spawn_status`), thử nghiệm Gateway hot-reload, bảo mật Cron, và 2 bản vá bảo mật. SylastraClaws đã đạt **25K Stars**!
...
2026-02-26 🎉 SylastraClaws đạt **20K Stars** chỉ trong 17 ngày! Tự động điều phối Channel và giao diện khả năng đã hoạt động.

2026-02-16 🎉 SylastraClaws vượt 12K Stars trong một tuần! Vai trò người duy trì cộng đồng và [Lộ trình](../../ROADMAP.md) chính thức ra mắt.

2026-02-13 🎉 SylastraClaws vượt 5000 Stars trong 4 ngày! Lộ trình dự án và nhóm nhà phát triển đang được xây dựng.

2026-02-09 🎉 **SylastraClaws ra mắt!** Được xây dựng trong 1 ngày để đưa AI Agent lên phần cứng $10 với <10MB RAM. Let's Go, SylastraClaws!
...
SylastraClaws có thể được triển khai trên hầu hết mọi thiết bị Linux!
...
### Tải xuống từ sylastraclaws.io (Khuyến nghị)

Truy cập **[sylastraclaws.io](https://sylastraclaws.io)** — website chính thức tự động phát hiện nền tảng của bạn và cung cấp tải xuống một cú nhấp. Không cần chọn kiến trúc thủ công.

### Tải xuống binary đã biên dịch sẵn

Ngoài ra, tải binary cho nền tảng của bạn từ trang [GitHub Releases](https://github.com/conglinyizhi/SylastraClaws/releases).
...
Sau khi tải xuống từ [sylastraclaws.io](https://sylastraclaws.io), nhấp đúp vào `sylastraclaws-launcher` (hoặc `sylastraclaws-launcher.exe` trên Windows). Trình duyệt của bạn sẽ tự động mở tại `http://localhost:18800`.
...
Để biết tài liệu WebUI chi tiết, xem [docs.sylastraclaws.io](https://docs.sylastraclaws.io).
...
> **Người dùng Docker / VM:** Gateway lắng nghe trên `127.0.0.1` theo mặc định. Đặt `SYLASTRACLAWS_GATEWAY_HOST=0.0.0.0` hoặc dùng cờ `-public` để có thể truy cập từ host.
...
macOS có thể chặn `sylastraclaws-launcher` khi khởi chạy lần đầu vì nó được tải từ internet và chưa được công chứng qua Mac App Store.

**Bước 1:** Nhấp đúp vào `sylastraclaws-launcher`. Bạn sẽ thấy cảnh báo bảo mật:

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="Cảnh báo macOS Gatekeeper" width="400">
</p>

> *"sylastraclaws-launcher" Không Mở Được — Apple không thể xác minh "sylastraclaws-launcher" không chứa phần mềm độc hại có thể gây hại cho Mac hoặc xâm phạm quyền riêng tư của bạn.*
...
Sau bước này, `sylastraclaws-launcher` sẽ mở bình thường trong các lần khởi chạy tiếp theo.
...
Hãy cho chiếc điện thoại cũ của bạn một cuộc sống mới! Biến nó thành Trợ lý AI thông minh với SylastraClaws.
...
Tải APK từ [sylastraclaws.io](https://sylastraclaws.io/download/) và cài đặt trực tiếp. Không cần Termux!
...
pkg install proot
termux-chroot ./sylastraclaws onboard   # chroot provides a standard Linux filesystem layout
```
...
Đối với các môi trường tối giản chỉ có binary lõi `sylastraclaws` (không có Launcher UI), bạn có thể cấu hình mọi thứ qua dòng lệnh và tệp cấu hình JSON.
...
Lệnh này tạo `~/.config/sylastraclaws/config.json` và thư mục workspace.

**2. Cấu hình** (`~/.config/sylastraclaws/config.json`)
...
SylastraClaws hỗ trợ 30+ Provider LLM thông qua cấu hình `model_list`. Sử dụng định dạng `protocol/model`:
...
Trò chuyện với SylastraClaws của bạn qua 17+ nền tảng nhắn tin:
...
> Mức độ chi tiết log được kiểm soát bởi `gateway.log_level` (mặc định: `warn`). Các giá trị được hỗ trợ: `debug`, `info`, `warn`, `error`, `fatal`. Cũng có thể đặt qua `SYLASTRACLAWS_LOG_LEVEL`. Xem [Cấu hình](../guides/configuration.vi.md#mức-log-của-gateway) để biết thêm chi tiết.
...
SylastraClaws có thể tìm kiếm web để cung cấp thông tin cập nhật. Cấu hình trong `tools.web`:
...
SylastraClaws bao gồm các tool tích hợp sẵn cho thao tác tệp, thực thi mã, lên lịch và nhiều hơn nữa. Xem [Cấu hình Tools](../reference/tools_configuration.vi.md) để biết chi tiết.
...
SylastraClaws hỗ trợ [MCP](https://modelcontextprotocol.io/) gốc — kết nối bất kỳ MCP server nào để mở rộng khả năng Agent của bạn với các tool và nguồn dữ liệu bên ngoài.
...
Kết nối SylastraClaws với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn duy nhất qua CLI hoặc bất kỳ Ứng dụng Chat nào đã tích hợp.
...
| Lệnh                      | Mô tả                                    |
| ------------------------- | ---------------------------------------- |
| `sylastraclaws onboard`        | Khởi tạo cấu hình & workspace           |
| `sylastraclaws auth weixin` | Kết nối tài khoản WeChat qua QR |
| `sylastraclaws agent -m "..."` | Trò chuyện với agent                     |
| `sylastraclaws agent`          | Chế độ trò chuyện tương tác             |
| `sylastraclaws gateway`        | Khởi động gateway                        |
| `sylastraclaws status`         | Hiển thị trạng thái                      |
| `sylastraclaws version`        | Hiển thị thông tin phiên bản            |
| `sylastraclaws model`          | Xem hoặc chuyển đổi mô hình mặc định   |
| `sylastraclaws cron list`      | Liệt kê tất cả công việc đã lên lịch   |
| `sylastraclaws cron add ...`   | Thêm công việc đã lên lịch             |
| `sylastraclaws cron disable`   | Vô hiệu hóa công việc đã lên lịch      |
| `sylastraclaws cron remove`    | Xóa công việc đã lên lịch              |
| `sylastraclaws skills list`    | Liệt kê các Skill đã cài đặt           |
| `sylastraclaws skills install` | Cài đặt một Skill                       |
| `sylastraclaws migrate`        | Di chuyển dữ liệu từ các phiên bản cũ  |
| `sylastraclaws auth login`     | Xác thực với các provider               |

### ⏰ Tác vụ Đã lên lịch / Nhắc nhở

SylastraClaws hỗ trợ nhắc nhở đã lên lịch và tác vụ định kỳ thông qua tool `cron`:
...
Xem [Lộ trình Cộng đồng](https://github.com/conglinyizhi/SylastraClaws/issues/988) và [CONTRIBUTING.md](../../CONTRIBUTING.md) để biết hướng dẫn.
