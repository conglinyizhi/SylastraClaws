...
<h1>SylastraClaws: Assistente de IA Ultra-Eficiente em Go</h1>

<h3>Hardware de $10 · 10MB de RAM · Boot em ms · Let's Go, SylastraClaws!</h3>
  <p>
...
> **SylastraClaws** é um projeto open-source independente iniciado pela [Sipeed](https://sipeed.com), escrito inteiramente em **Go** do zero — não é um fork do OpenClaw, NanoBot ou qualquer outro projeto.

**SylastraClaws** é um assistente de IA pessoal ultra-leve inspirado no [NanoBot](https://github.com/HKUDS/nanobot). Foi reconstruído do zero em **Go** por meio de um processo de "auto-bootstrapping" — o próprio AI Agent conduziu a migração de arquitetura e a otimização do código.
...
> **Aviso de Segurança**
>
> * **SEM CRIPTO:** O SylastraClaws **não** emitiu nenhum token oficial ou criptomoeda. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **golpes**.
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é **[sylastraclaws.io](https://sylastraclaws.io)**, e o site da empresa é **[sipeed.com](https://sipeed.com)**
> * **ATENÇÃO:** Muitos domínios `.ai/.org/.com/.net/...` foram registrados por terceiros. Não confie neles.
> * **NOTA:** O SylastraClaws está em desenvolvimento rápido inicial. Podem existir problemas de segurança não resolvidos. Não implante em produção antes da v1.0.
> * **NOTA:** O SylastraClaws mesclou muitos PRs recentemente. Builds recentes podem usar 10-20MB de RAM. A otimização de recursos está planejada após a estabilização de funcionalidades.

## 📢 Novidades

2026-03-31 📱 **Suporte Android!** SylastraClaws agora roda no Android! Baixe o APK em [sylastraclaws.io](https://sylastraclaws.io/download)

2026-03-25 🚀 **v0.2.4 Lançada!** Reformulação da arquitetura Agent (SubTurn, Hooks, Steering, EventBus), integração WeChat/WeCom, fortalecimento de segurança (.security.yml, filtragem de dados sensíveis), novos providers (AWS Bedrock, Azure, Xiaomi MiMo) e 35 correções de bugs. O SylastraClaws atingiu **26K Stars**!

2026-03-17 🚀 **v0.2.3 Lançada!** UI na bandeja do sistema (Windows e Linux), consulta de status de sub-agent (`spawn_status`), hot-reload experimental do Gateway, controle de segurança do Cron e 2 correções de segurança. O SylastraClaws atingiu **25K Stars**!
...
2026-02-26 🎉 O SylastraClaws atinge **20K Stars** em apenas 17 dias! Orquestração automática de channels e interfaces de capacidade estão disponíveis.

2026-02-16 🎉 O SylastraClaws ultrapassa 12K Stars em uma semana! Funções de mantenedor da comunidade e [Roadmap](../../ROADMAP.md) lançados oficialmente.

2026-02-13 🎉 O SylastraClaws ultrapassa 5000 Stars em 4 dias! Roadmap do projeto e grupos de desenvolvedores em andamento.

2026-02-09 🎉 **SylastraClaws Lançado!** Construído em 1 dia para levar AI Agents a hardware de $10 com menos de 10MB de RAM. Let's Go, SylastraClaws!
...
O SylastraClaws pode ser implantado em praticamente qualquer dispositivo Linux!
...
### Download pelo sylastraclaws.io (Recomendado)

Acesse **[sylastraclaws.io](https://sylastraclaws.io)** — o site oficial detecta automaticamente sua plataforma e fornece download com um clique. Não é necessário selecionar a arquitetura manualmente.

### Download do binário pré-compilado

Alternativamente, baixe o binário para sua plataforma na página de [GitHub Releases](https://github.com/conglinyizhi/SylastraClaws/releases).
...
Após baixar de [sylastraclaws.io](https://sylastraclaws.io), dê duplo clique em `sylastraclaws-launcher` (ou `sylastraclaws-launcher.exe` no Windows). Seu navegador abrirá automaticamente em `http://localhost:18800`.
...
Para documentação detalhada do WebUI, veja [docs.sylastraclaws.io](https://docs.sylastraclaws.io).
...
> **Usuários de Docker / VM:** O Gateway escuta em `127.0.0.1` por padrão. Defina `SYLASTRACLAWS_GATEWAY_HOST=0.0.0.0` ou use a flag `-public` para torná-lo acessível pelo host.
...
O macOS pode bloquear o `sylastraclaws-launcher` no primeiro lançamento porque ele foi baixado da internet e não é notarizado pela Mac App Store.

**Passo 1:** Dê um duplo clique em `sylastraclaws-launcher`. Você verá um aviso de segurança:

<p align="center">
<img src="../../assets/macos-gatekeeper-warning.jpg" alt="Aviso do macOS Gatekeeper" width="400">
</p>

> *"sylastraclaws-launcher" não foi aberto — A Apple não conseguiu verificar se "sylastraclaws-launcher" está livre de malware que possa prejudicar seu Mac ou comprometer sua privacidade.*
...
Após esta etapa única, o `sylastraclaws-launcher` abrirá normalmente nos lançamentos seguintes.
...
Dê uma segunda vida ao seu celular de uma década! Transforme-o em um Assistente de IA inteligente com o SylastraClaws.
...
Baixe o APK de [sylastraclaws.io](https://sylastraclaws.io/download/) e instale diretamente. Sem necessidade de Termux!
...
pkg install proot
termux-chroot ./sylastraclaws onboard   # chroot fornece um layout padrão de sistema de arquivos Linux
```
...
Para ambientes mínimos onde apenas o binário principal `sylastraclaws` está disponível (sem Launcher UI), você pode configurar tudo via linha de comando e um arquivo de configuração JSON.
...
Isso cria `~/.config/sylastraclaws/config.json` e o diretório workspace.

**2. Configurar** (`~/.config/sylastraclaws/config.json`)
...
O SylastraClaws suporta mais de 30 providers de LLM através da configuração `model_list`. Use o formato `protocolo/modelo`:
...
Converse com seu SylastraClaws por meio de mais de 17 plataformas de mensagens:
...
> A verbosidade dos logs é controlada por `gateway.log_level` (padrão: `warn`). Valores suportados: `debug`, `info`, `warn`, `error`, `fatal`. Também pode ser definido via `SYLASTRACLAWS_LOG_LEVEL`. Veja [Configuração](../guides/configuration.pt-br.md#nível-de-log-do-gateway) para detalhes.
...
O SylastraClaws pode pesquisar na web para fornecer informações atualizadas. Configure em `tools.web`:
...
O SylastraClaws inclui ferramentas integradas para operações de arquivo, execução de código, agendamento e mais. Veja [Configuração de Ferramentas](../reference/tools_configuration.pt-br.md) para detalhes.
...
O SylastraClaws suporta nativamente o [MCP](https://modelcontextprotocol.io/) — conecte qualquer servidor MCP para estender as capacidades do seu Agent com ferramentas externas e fontes de dados.
...
Conecte o SylastraClaws à Rede Social de Agents simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.
...
| Comando                   | Descrição                              |
| ------------------------- | -------------------------------------- |
| `sylastraclaws onboard`        | Inicializar config e workspace         |
| `sylastraclaws auth weixin` | Conectar conta WeChat via QR |
| `sylastraclaws agent -m "..."` | Conversar com o agent                  |
| `sylastraclaws agent`          | Modo de chat interativo                |
| `sylastraclaws gateway`        | Iniciar o gateway                      |
| `sylastraclaws status`         | Exibir status                          |
| `sylastraclaws version`        | Exibir informações de versão           |
| `sylastraclaws model`          | Ver ou trocar o modelo padrão          |
| `sylastraclaws cron list`      | Listar todos os jobs agendados         |
| `sylastraclaws cron add ...`   | Adicionar um job agendado              |
| `sylastraclaws cron disable`   | Desabilitar um job agendado            |
| `sylastraclaws cron remove`    | Remover um job agendado                |
| `sylastraclaws skills list`    | Listar skills instaladas               |
| `sylastraclaws skills install` | Instalar uma skill                     |
| `sylastraclaws migrate`        | Migrar dados de versões anteriores     |
| `sylastraclaws auth login`     | Autenticar com providers               |

### ⏰ Tarefas Agendadas / Lembretes

O SylastraClaws suporta lembretes agendados e tarefas recorrentes através da ferramenta `cron`:
...
Veja nosso [Roadmap da Comunidade](https://github.com/conglinyizhi/SylastraClaws/issues/988) e [CONTRIBUTING.md](../../CONTRIBUTING.md) para diretrizes.
