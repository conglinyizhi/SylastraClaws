...
**Antigravity** (Google Cloud Code Assist) é um provedor de modelos de IA apoiado pelo Google que oferece acesso a modelos como Claude Opus 4.6 e Gemini através da infraestrutura de nuvem do Google. Este documento fornece um guia completo sobre como a autenticação funciona, como buscar modelos e como implementar um novo provedor no SylastraClaws.
...
10. [Criando um Novo Provedor no SylastraClaws](#criando-um-novo-provedor-no-sylastraclaws)
...
const antigravityPlugin = {
  ...
  register(api: SylastraClawsPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<SylastraClawsConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.25
- Base de código do SylastraClaws (`pkg/providers/` e `pkg/auth/`)
...
Os perfis de autenticação são armazenados em `~/.config/sylastraclaws/auth.json`:
...
## Criando um Novo Provedor no SylastraClaws

Os provedores do SylastraClaws são implementados como pacotes Go em `pkg/providers/`. Para adicionar um novo provedor:
...
Se seu provedor requer OAuth ou autenticação especial, adicione um caso em `cmd/sylastraclaws/internal/auth/helpers.go`:
...
sylastraclaws auth login --provider your-provider

# Listar modelos (para Antigravity)
sylastraclaws auth models

# Iniciar o gateway
sylastraclaws gateway

# Executar um agente com um modelo específico
sylastraclaws agent -m "Hello" --model your-model
```
...
export SYLASTRACLAWS_AGENTS_DEFAULTS_MODEL=your-model

# Substituir configurações do provedor
export SYLASTRACLAWS_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
- **Arquivos Fonte:**
  ...
  - `pkg/auth/store.go` - Armazenamento de credenciais de autenticação (`~/.config/sylastraclaws/auth.json`)
  ...
  - `cmd/sylastraclaws/internal/auth/helpers.go` - Comandos CLI de autenticação
...
- Atualizar tokens OAuth: `sylastraclaws auth login --provider antigravity`
...
- Verificar o armazenamento do perfil de autenticação: `~/.config/sylastraclaws/auth.json`
- Executar novamente `sylastraclaws auth login --provider antigravity`
