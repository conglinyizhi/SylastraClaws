...
10. [Créer un nouveau fournisseur dans SylastraClaws](#créer-un-nouveau-fournisseur-dans-sylastraclaws)
...
- Go ≥ 1.25
- Base de code SylastraClaws (`pkg/providers/` et `pkg/auth/`)
...
Les profils d'authentification sont stockés dans `~/.config/sylastraclaws/auth.json` :
...
## Créer un nouveau fournisseur dans SylastraClaws

Les fournisseurs SylastraClaws sont implémentés en tant que packages Go sous `pkg/providers/`. Pour ajouter un nouveau fournisseur :
...
Si votre fournisseur nécessite OAuth ou une authentification spéciale, ajoutez un cas dans `cmd/sylastraclaws/internal/auth/helpers.go` :
...
- **Fichiers source :**
  ...
  - `pkg/auth/store.go` - Stockage des identifiants d'authentification (`~/.config/sylastraclaws/auth.json`)
  ...
  - `cmd/sylastraclaws/internal/auth/helpers.go` - Commandes CLI d'authentification
...
- Rafraîchir les jetons OAuth : `sylastraclaws auth login --provider antigravity`
...
- Vérifier le stockage du profil d'authentification : `~/.config/sylastraclaws/auth.json`
- Relancer `sylastraclaws auth login --provider antigravity`
