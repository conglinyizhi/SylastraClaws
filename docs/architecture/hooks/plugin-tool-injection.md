...
This document demonstrates how to use SylastraClaws's hook system to implement external plugin tool injection, allowing LLM to call tools implemented by external hook processes.
...
This way, external hooks can fully implement plugin tools without registering any tools inside SylastraClaws.
...
1. SylastraClaws sends `hook.before_llm`, hook injects `get_weather` tool definition
2. LLM sees tool definition, decides to call `get_weather(city="Beijing")`
3. SylastraClaws sends `hook.before_tool`, hook uses `respond` action to return weather data
...
These references are managed by SylastraClaws's MediaStore and can be:
...
Injected plugin tools coexist with SylastraClaws built-in tools:
...
3. **Coexist with built-in tools**: Does not affect normal operation of SylastraClaws's original tools
