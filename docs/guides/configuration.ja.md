...
設定ファイルパス: `~/.config/sylastraclaws/config.json`

### 環境変数

環境変数を使用してデフォルトパスを上書きできます。ポータブルインストール、コンテナ化デプロイ、または sylastraclaws をシステムサービスとして実行する場合に便利です。これらの変数は独立しており、異なるパスを制御します。

| 変数              | 説明                                                                                                                             | デフォルトパス            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SYLASTRACLAWS_CONFIG` | 設定ファイルのパスを上書きします。sylastraclaws がどの `config.json` を読み込むかを直接指定し、他のすべての場所を無視します。 | `~/.config/sylastraclaws/config.json` |
| `SYLASTRACLAWS_HOME`   | sylastraclaws データのルートディレクトリを上書きします。`workspace` やその他のデータディレクトリのデフォルト場所を変更します。          | `~/.config/sylastraclaws`             |

**例：**

```bash
# 特定の設定ファイルで sylastraclaws を実行
# ワークスペースパスはその設定ファイル内から読み込まれます
SYLASTRACLAWS_CONFIG=/etc/sylastraclaws/production.json sylastraclaws gateway

# /opt/sylastraclaws にすべてのデータを保存して sylastraclaws を実行
# 設定はデフォルトの ~/.config/sylastraclaws/config.json から読み込まれます
# ワークスペースは /opt/sylastraclaws/workspace に作成されます
...
環境変数でも上書き可能です：`SYLASTRACLAWS_LOG_LEVEL=info`

### ワークスペースレイアウト

SylastraClaws は設定されたワークスペース（デフォルト: `~/.config/sylastraclaws/workspace`）にデータを保存します：
...
1. `~/.config/sylastraclaws/workspace/skills`（ワークスペース）
2. `~/.config/sylastraclaws/skills`（グローバル）
...
SylastraClaws はデフォルトでサンドボックス環境で実行されます。Agent は設定されたワークスペース内のファイルアクセスとコマンド実行のみが可能です。
...
| オプション              | デフォルト値            | 説明                                  |
| ----------------------- | ----------------------- | ------------------------------------- |
| `workspace`             | `~/.config/sylastraclaws/workspace` | Agent の作業ディレクトリ              |
...
exec セキュリティガードは SylastraClaws が直接起動するコマンドラインのみを検査します。`make`、`go run`、`cargo`、`npm run`、またはカスタムビルドスクリプトなどの開発ツールが生成する子プロセスは再帰的に検査しません。
...
* ビルトインガードより強力な分離が必要な場合は、コンテナまたは VM 内で SylastraClaws を実行してください。
...
SylastraClaws は定期タスクを自動実行できます。ワークスペースに `HEARTBEAT.md` ファイルを作成してください：
...
- `SYLASTRACLAWS_HEARTBEAT_ENABLED=false` で無効化
- `SYLASTRACLAWS_HEARTBEAT_INTERVAL=60` で間隔を変更
...
| `volcengine` | LLM（Volcengine 直接）                  | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=SylastraClaws&utm_content=SylastraClaws&utm_medium=devrel&utm_source=OWO&utm_term=SylastraClaws) |
...
> **新機能：** SylastraClaws は**モデル中心**の設定アプローチを採用しました。`vendor/model` 形式（例：`zhipu/glm-4.7`）を指定するだけで新しい Provider を追加できます — **コード変更不要！**
...
| **VolcEngine (Doubao)** | `volcengine/`          | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI     | [取得](https://www.volcengine.com/activity/codingplan?utm_campaign=SylastraClaws&utm_content=SylastraClaws&utm_medium=devrel&utm_source=OWO&utm_term=SylastraClaws) |
| **Antigravity**         | `antigravity/`         | Google Cloud                                        | Custom     | OAuth のみ                                                       |

#### ロードバランシング

同じモデル名に複数のエンドポイントを設定すると、SylastraClaws が自動的にラウンドロビンします：
...
SylastraClaws はプロトコルファミリーで Provider をルーティングします：
...
SylastraClaws は `cron` ツールを通じて cron スタイルのスケジュールタスクをサポートします。
...
スケジュールタスクは再起動後も `~/.config/sylastraclaws/workspace/cron/` に保存されます。
