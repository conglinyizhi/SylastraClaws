...
Converse com seu sylastraclaws através do Telegram, Discord, WhatsApp, Matrix, QQ, DingTalk, LINE, WeCom, Feishu, Slack, IRC, OneBot ou MaixCam
...
| **Pico**             | ⭐ Fácil           | Canal de protocolo nativo SylastraClaws                    |                                                                                                                  |
...
O SylastraClaws agora mantém definições de comandos em um registro compartilhado. Na inicialização, o Telegram registrará automaticamente os comandos de bot suportados (por exemplo `/start`, `/help`, `/show`, `/list`, `/use`, `/btw`) para que o menu de comandos e o comportamento em tempo de execução permaneçam sincronizados.
...
Se o registro de comandos falhar (erros transitórios de rede/API), o canal ainda inicia e o SylastraClaws tenta novamente o registro em segundo plano.
...
O SylastraClaws pode se conectar ao WhatsApp de duas formas:
...
Se `session_store_path` estiver vazio, a sessão é armazenada em `<workspace>/whatsapp/`. Execute `sylastraclaws gateway`; na primeira execução, escaneie o QR code impresso no terminal com WhatsApp → Dispositivos Vinculados.
...
O SylastraClaws suporta conexão com sua conta pessoal do WeChat usando a API oficial Tencent iLink.
...
4. Execute `sylastraclaws gateway` e abra o QQ para conversar com seu bot
...
* Configure conforme mostrado acima e execute `sylastraclaws gateway`
...
O SylastraClaws suporta três tipos de integração WeCom:
...
O SylastraClaws se conecta ao Feishu via modo WebSocket/SDK — não é necessário URL de webhook público nem servidor de callback.
...
OneBot é um protocolo aberto para bots QQ. O SylastraClaws se conecta a qualquer implementação compatível com OneBot v11 (ex.: [Lagrange](https://github.com/LagrangeDev/Lagrange.Core), [NapCat](https://github.com/NapNeko/NapCatQQ)) via WebSocket.
