...
配置文件路径: `~/.config/sylastraclaws/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 sylastraclaws 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `SYLASTRACLAWS_CONFIG` | 覆盖配置文件的路径。这直接告诉 sylastraclaws 加载哪个 `config.json`，忽略所有其他位置。 | `~/.config/sylastraclaws/config.json` |
| `SYLASTRACLAWS_HOME`   | 覆盖 sylastraclaws 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.config/sylastraclaws`             |

**示例：**

```bash
# 使用特定的配置文件运行 sylastraclaws
# 工作区路径将从该配置文件中读取
SYLASTRACLAWS_CONFIG=/etc/sylastraclaws/production.json sylastraclaws gateway

# 在 /opt/sylastraclaws 中存储所有数据运行 sylastraclaws
# 配置将从默认的 ~/.config/sylastraclaws/config.json 加载
# 工作区将在 /opt/sylastraclaws/workspace 创建
...
也可通过环境变量覆盖：`SYLASTRACLAWS_LOG_LEVEL=info`

### 工作区布局 (Workspace Layout)

SylastraClaws 将数据存储在您配置的工作区中（默认：`~/.config/sylastraclaws/workspace`）：
...
用 **sylastraclaws-launcher** 打开浏览器控制台前需要先使用密码登录。首次启动时打开 `/launcher-setup` 创建 dashboard 登录密码；后续手动登录使用 `/launcher-login`。

- **配置文件**：与 `config.json` 同一目录（若设置了 `SYLASTRACLAWS_CONFIG`，则与它所指的文件同目录）。启动器专用文件名为 `launcher-config.json`。
...
- **不再支持的鉴权方式**：不再支持 URL token 登录（`?token=...`）、`SYLASTRACLAWS_LAUNCHER_TOKEN` 和 `Authorization: Bearer` dashboard 鉴权。
...
1. `~/.config/sylastraclaws/workspace/skills`（工作区）
2. `~/.config/sylastraclaws/skills`（全局）
3. `<构建时嵌入路径>/skills`（内置）
...
规则按从上到下顺序检查，第一条命中的规则立即生效。若没有规则命中，SylastraClaws 会回退到默认 agent。
...
SylastraClaws 默认在沙箱环境中运行。Agent 只能访问配置的工作区内的文件和执行命令。
...
| 选项                    | 默认值                  | 描述                          |
| ----------------------- | ----------------------- | ----------------------------- |
| `workspace`             | `~/.config/sylastraclaws/workspace` | Agent 的工作目录              |
...
exec 安全守卫仅检查 SylastraClaws 直接启动的命令行。它不会递归检查由 `make`、`go run`、`cargo`、`npm run` 或自定义构建脚本等开发工具产生的子进程。
...
* 如果需要比内置守卫更强的隔离，请在容器或虚拟机中运行 SylastraClaws。
...
SylastraClaws 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `SYLASTRACLAWS_HEARTBEAT_ENABLED=false` 禁用
- `SYLASTRACLAWS_HEARTBEAT_INTERVAL=60` 更改间隔
...
| `volcengine` | LLM（火山引擎直连）                     | [volcengine.com](https://www.volcengine.com/activity/codingplan?utm_campaign=SylastraClaws&utm_content=SylastraClaws&utm_medium=devrel&utm_source=OWO&utm_term=SylastraClaws) |
...
> **新特性：** SylastraClaws 现在优先推荐显式 `provider` + 原生 `model` 的配置方式，例如 `"provider": "zhipu", "model": "glm-4.7"`。如果未设置 `provider`，旧的单字段 `provider/model` 写法仍然兼容。
...
| **火山引擎 (豆包)**     | `volcengine`      | `https://ark.cn-beijing.volces.com/api/v3`          | OpenAI    | [获取](https://www.volcengine.com/activity/codingplan?utm_campaign=SylastraClaws&utm_content=SylastraClaws&utm_medium=devrel&utm_source=OWO&utm_term=SylastraClaws) |
...
- 如果设置了 `provider`，SylastraClaws 会将 `model` 原样发送。
- 如果未设置 `provider`，SylastraClaws 会把 `model` 第一个 `/` 之前的字段当作 provider，并把第一个 `/` 之后的全部内容当作最终模型 ID。
...
> 运行 `sylastraclaws auth login --provider anthropic` 粘贴 API Token。
...
显式设置 `provider` 后，SylastraClaws 会把 `openai/gpt-oss-20b` 原样发送给 LM Studio。旧的兼容写法 `"model": "lmstudio/openai/gpt-oss-20b"` 在未设置 `provider` 时也会解析成相同的上游模型 ID。
...
显式设置 `provider` 后，SylastraClaws 会将 `model` 原样发送。因此 `"provider": "litellm", "model": "lite-gpt4"` 会发送 `lite-gpt4`，而 `"provider": "litellm", "model": "openai/gpt-4o"` 会发送 `openai/gpt-4o`。旧的兼容写法 `litellm/lite-gpt4` 和 `litellm/openai/gpt-4o` 在未设置 `provider` 时也会得到相同结果。

</details>

#### 负载均衡

为同一模型名称配置多个端点，SylastraClaws 会自动轮询：
...
SylastraClaws 通过 `cron` 工具支持 cron 风格的定时任务。Agent 可以设置、列出和取消在指定时间触发的提醒或周期性任务。
...
定时任务在重启后持久保存，存储于 `~/.config/sylastraclaws/workspace/cron/`。
