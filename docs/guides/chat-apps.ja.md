...
SylastraClaws は複数のチャットプラットフォームをサポートしており、Agent をどこにでも接続できます。
...
| **Pico**             | ⭐ 簡単            | SylastraClaws ネイティブプロトコルチャネル     |                                                                                                               |
...
SylastraClaws は統一されたコマンド定義を使用します。起動時に Telegram がサポートするコマンド（例: `/start`、`/help`、`/show`、`/list`、`/use`、`/btw`）を Bot コマンドメニューに自動登録し、メニュー表示と実際の動作を一致させます。
...
SylastraClaws は 2 つの WhatsApp 接続方式をサポートしています：
...
`session_store_path` が空の場合、セッションは `<workspace>/whatsapp/` に保存されます。`sylastraclaws gateway` を実行し、初回実行時にターミナルに表示される QR コードをスキャンしてください（WhatsApp → リンクデバイス）。
...
SylastraClaws は Tencent iLink 公式 API を使用して WeChat 個人アカウントへの接続をサポートしています。
...
4. `sylastraclaws gateway` を実行し、QQ を開いてボットとチャット
...
* 上記の設定を行い、`sylastraclaws gateway` を実行
...
SylastraClaws は WebSocket/SDK モードで飛書に接続します — 公開 Webhook URL やコールバックサーバーは不要です。
...
SylastraClaws は 3 種類の WeCom 統合をサポートしています：
...
OneBot は QQ ボット向けのオープンプロトコルです。SylastraClaws は OneBot v11 互換の実装（例：[Lagrange](https://github.com/LagrangeDev/Lagrange.Core)、[NapCat](https://github.com/NapNeko/NapCatQQ)）に WebSocket で接続します。
